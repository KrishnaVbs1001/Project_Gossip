...
actor Main
  ...
  fun usage() =>
    _env.out.print("Command Usage: program <Gossip/Pushsum> <number_of_nodes> <topology>")
    _env.exitcode(1)
  ...
  be node_updated(id: String, is_informed: Bool = false) =>
    if not _is_complete then
      if is_informed then
        ...
        _env.out.print("Node " + id + " terminated. Total: " + _stopped_count.string() + "/" + _total_nodes.string())
  ...
  be check_progress() =>
    if not _is_complete then
      _env.out.print("Progress: " + _informed_count.string() + "/" + _total_nodes.string() + " nodes informed, " +
                     _stopped_count.string() + "/" + _total_nodes.string() + " nodes terminated")
    end
  ...
  be algorithm_complete() =>
    if not _is_complete then
      ...
      _env.out.print("Algorithm complete. Total nodes terminated: " + _stopped_count.string() + "/" + _total_nodes.string())
      _timers.dispose()
  ...
  be timeout() =>
    if not _is_complete then
      _is_complete = true
      _env.out.print("Timeout reached. Nodes informed: " + _stopped_count.string() + "/" + _total_nodes.string())
      _timers.dispose()
      _env.out.print("Exiting program")
